...
DataBlock* Module_v1731::GetModuleBuffer(){  
	...
	int nsample = 2000;
	//int nr_elem = 1024;
	int nr_elem = 1024;  // 4 samples per DWORD, 2 CHs and 4 DWORDs for the header
...
printf("looping through %d lines\n",nr_elem);
for(int i = 0; i<nr_elem; i++){


//Event size
unsigned int check1 = (unsigned int) ((rawdata[i]&(1<<31))>>31);
unsigned int check2 = (unsigned int) ((rawdata[i]&(1<<30))>>30);
unsigned int check3 = (unsigned int) ((rawdata[i]&(1<<29))>>29);
unsigned int check4 = (unsigned int) ((rawdata[i]&(1<<28))>>28);

if(check1 ==1 && check2 ==0 && check3 ==1 && check4 ==0){
